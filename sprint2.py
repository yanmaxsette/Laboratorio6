...
while(count < 20):
	data = '{"query": "{ search(query:\\"stars:>100\\", type:REPOSITORY, first:50, after:'+endCursor+'){ pageInfo {hasNextPage endCursor} nodes { ... on Repository {nameWithOwner url createdAt updatedAt pullRequests{ totalCount } releases{ totalCount } primaryLanguage{ name } all_issues: issues{ totalCount } closed_issues: issues(states:CLOSED){ totalCount } } } } rateLimit {remaining resetAt}} "}'
